...
def wrap_featurehub(
    ...
    symbol: str,
    set_name: str,
    original_fe_fn: Callable[[], pd.DataFrame],
    ...
    Args:
        strategy_name: 策略名（如 "btc", "alt_trend", "equity_classic"）
        ohlcv_df: OHLCV 数据
        symbol: 交易标的
        set_name: FeatureHub 启用集合名
        original_fe_fn: 原始 FE 函数（无参数，返回 DataFrame）
        macro_df: 可选宏观数据
        ref_df: 可选参考资产数据
    ...
    try:
        ...
        logger.info(
            "[H3] %s → FeatureHub ok: %d features from %s (strip_prefix=%s)",
            strategy_name,
            len(df_out.columns),
