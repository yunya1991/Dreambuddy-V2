...
class EvolutionPipeline:
    ...
    def __init__(self, gene_root: Path | str | None = None):
        self.gene_root = Path(gene_root) if gene_root else (
            REPO_ROOT / "4-MEMORY" / "2-交易记忆单元"
        )
        # 路径注入
        for p in [
            str(REPO_ROOT),
            str(REPO_ROOT / "21-特征工程中心" / "feature_hub"),
            str(REPO_ROOT / "4-MEMORY" / "2-交易记忆单元"),
        ]:
            if p not in sys.path:
                sys.path.insert(0, p)

        # L3/L4 初始化
        from shadow_rl import ShadowRLTracker
        from bellman_tracker import BellmanVTracker
    ...
    def _load_library(self):
        if self._library is None:
            import strategy_gene_schema as sgs
    ...
    def run_symbol(self, symbol: str, market_data: dict, rv=None) -> dict[str, Any]:
        ...
        """
        from resistance_features import ResistanceVector
        from level0_path_cost import compute_g_diag, compute_d_star
        import strategy_gene_schema as sgs
