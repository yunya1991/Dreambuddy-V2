...
import { ExecutionPlanner } from './planner/planner';
import { ensureRegistryInitialized } from './planner/skills-registry-init';
import type { PlannerContext, PlannerExecutionResult, PlannerProgressEvent } from './planner/planner-types';
...
} from './planner/supplement-memory-store';
import {
...
// 设计原则：
// 1. 短对话（<= 5条）：保留完整历史
// 2. 中对话（5-10条）：保留最近3条完整 + 早期摘要
// 3. 长对话（> 10条）：使用图压缩（节点摘要 + 关键关系）
// ============================================================
...
const COMPRESSION_INTERVAL_MS = 5 * 60 * 1000;
...
export function addConversationTurn(
  ...
  if (!ctx) {
    ctx = { turns: [], compressedSummary: '', lastCompressedAt: 0 };
  ...
  if (ctx.turns.length >= COMPRESSION_THRESHOLDS.LIGHT) {
    compressContext(ctx);
  }
}

function compressContext(ctx: SessionContext): void {
  const now = Date.now();
  if (now - ctx.lastCompressedAt < COMPRESSION_INTERVAL_MS && ctx.compressedSummary) {
    return;
  }
  ...
  if (totalTurns <= COMPRESSION_THRESHOLDS.LIGHT) {
    ctx.compressedSummary = '';
    ctx.lastCompressedAt = now;
    return;
  }

  if (totalTurns <= COMPRESSION_THRESHOLDS.MEDIUM) {
    const earlyTurns = turns.slice(0, -3);
    ctx.compressedSummary = buildEarlyTurnsSummary(earlyTurns);
    ctx.lastCompressedAt = now;
    return;
  }

  ctx.compressedSummary = buildGraphCompression(turns);
  ctx.lastCompressedAt = now;
}

function buildEarlyTurnsSummary(turns: ConversationTurn[]): string {
  const intents = [...new Set(turns.map(t => t.intent))];
  const symbols = extractSymbolsFromTurns(turns);
  
  let summary = `【早期对话摘要】`;
  if (symbols.length > 0) {
    summary += ` 讨论的品种: ${symbols.join(', ')}`;
  }
  if (intents.length > 0) {
    summary += ` | 分析类型: ${intents.join(', ')}`;
  }
  summary += ` | 共 ${turns.length} 轮对话`;
  
  return summary;
}

function buildGraphCompression(turns: ConversationTurn[]): string {
  const nodes: Record<string, { count: number; summaries: string[] }> = {};
  const edges: string[] = [];
  ...
  for (const turn of turns) {
    ...
    nodes[intent].count++;
    nodes[intent].summaries.push(turn.response.slice(0, 60));

    if (symbols) {
      edges.push(`${intent} → ${symbols.symbol}`);
    }
  }

  let graphSummary = `【对话图压缩】\n`;
  
  graphSummary += `\n📊 意图节点:\n`;
  for (const [intent, info] of Object.entries(nodes)) {
    graphSummary += `  - ${intent}: ${info.count}次\n`;
  }
  ...
  for (const t of recentTurns) {
    graphSummary += `  - 用户: ${t.message.slice(0, 40)}... → ${t.intent}\n`;
  }

  return graphSummary;
}

function extractSymbolsFromTurns(turns: ConversationTurn[]): string[] {
  const symbols = new Set<string>();
  for (const turn of turns) {
    const symbolDef = extractSymbolFromMessage(turn.message);
    if (symbolDef?.symbol) {
      symbols.add(symbolDef.symbol);
    }
  }
  return Array.from(symbols);
}

export function getCompressedContext(sessionId: string): string[] {
  const ctx = sessionContexts.get(sessionId);
  if (!ctx || ctx.turns.length === 0) {
    return [];
  }

  const totalTurns = ctx.turns.length;

  if (totalTurns <= COMPRESSION_THRESHOLDS.LIGHT) {
    return ctx.turns.map(t => `用户: ${t.message}\n系统: ${t.response}`);
  }
  ...
  if (ctx.compressedSummary) {
    contextLines.push(ctx.compressedSummary);
  }

  for (const t of recentTurns) {
    contextLines.push(`用户: ${t.message}\n系统: ${t.response.slice(0, 200)}`);
  }
...
export async function createAndExecuteTask(params: {
  ...
  // 获取智能路由
  // 使用动态上下文压缩：保留完整对话历史，但过长时进行图压缩
  const compressedContext = getCompressedContext(task.session_id);
  const messageHistory = [...compressedContext, params.message];
  
  console.log(`[ContextCompression] Session ${task.session_id}: ${compressedContext.length} compressed items, total history: ${messageHistory.length}`);
  ...
  if (isConversationIntent(intentType)) {
    ...
    if (result) {
      addConversationTurn(task.session_id, params.message, result.content || '', intentType, task.task_id);
    }
    return { task, result, needAsync: false };
  }
