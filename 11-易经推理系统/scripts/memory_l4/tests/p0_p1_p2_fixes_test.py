...
def test_p1_a_floor_uses_configurable_threshold():
    ...
    assert (
        "A_CONFIDENCE_FLOOR = 0.7955" not in src
    ...
    assert has_threshold_ref, "P1 FAIL：A项过滤未使用可配置阈值"
    assert (
        "0.40" in src or "A_SAFETY_FLOOR" in src or "safety_floor" in src
...
def test_p2_place_order_failure_exposes_proxies():
    ...
    assert "proxies" in src, "P2 FAIL：place_order 失败分支未包含 proxies 信息"
    assert "proxies=" in src, "P2 FAIL：error 字段未包含 proxies= 诊断"
...
def test_p4_xaut_to_xau_normalization():
    ...
    assert (
        "XAUT" not in coins_default_list
    ), f"P4 FAIL(A)：--coins default 仍含 XAUT（不存在的合约）-> {coins_default_list}"
    ...
    assert not wrong_back, "P4 FAIL(C)：仍存在错误的 XAU→XAUT 反向映射，会把存在的合约改错！"
...
def test_p0_proxy_assignment_and_probe():
    ...
    assert (
        "self.session.proxies = dict(proxies)" in src_setup
        or "self.session.proxies = {" in src_setup
    ), "P0 FAIL：_proxy_setup 中未使用直接赋值确保代理生效"
    # 必须调用 probe
    assert (
        "_probe_proxy_or_log" in src_setup
    ), "P0 FAIL：_proxy_setup 后未调用 _probe_proxy_or_log 诊断"
    # 类中必须有 probe 方法
    assert hasattr(
        OKXSimulatedClient, "_probe_proxy_or_log"
...
def test_p0_bcrm2_fetch_okx_klines_has_proxy_and_swap_fallback():
    ...
    # 代理相关关键词
    has_env_proxy = any(
        k in src for k in ["HTTPS_PROXY", "HTTP_PROXY", "ALL_PROXY", "os.environ.get("]
    )
    has_session_or_proxies = "Session(" in src and "proxies" in src
    assert (
        has_env_proxy and has_session_or_proxies
    ), "P0 FAIL：bcrm2.fetch_okx_klines 未读取 os.environ 代理或未新建 Session 配置 proxies"
    # 必须有 51001/SWAP fallback
    has_swap_fallback = ("-USDT-SWAP" in src or "'-USDT-SWAP'" in src) and (
        "51001" in src or "candidates" in src or "inst_id" in src
    )
    assert (
        has_swap_fallback
...
def test_p0_bcrm2_get_klines_spot_and_xau_swap_smoke():
    ...
    assert len(df_xaut) > 0, "P4(bcrm2 smoke)：XAUT 输入未规范化为 XAU，未返回有效 K线"
...
if __name__ == "__main__":
    ...
    test(
        "P0/P4 - bcrm2 fetch_okx_klines代理+SWAP fallback代码",
        test_p0_bcrm2_fetch_okx_klines_has_proxy_and_swap_fallback,
    )
