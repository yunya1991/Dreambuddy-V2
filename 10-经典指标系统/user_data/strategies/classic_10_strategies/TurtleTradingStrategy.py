...
class TurtleTradingStrategy(IStrategy):
    ...
    entry_lookback_fast = IntParameter(10, 40, default=20, space="buy")
    entry_lookback_slow = IntParameter(20, 120, default=55, space="buy")
    exit_lookback = IntParameter(5, 30, default=10, space="sell")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        fast = int(self.entry_lookback_fast.value)
        slow = int(self.entry_lookback_slow.value)
        ex = int(self.exit_lookback.value)

        dataframe["donchian_high_fast"] = high.rolling(fast).max().shift(1)
        dataframe["donchian_low_fast"] = low.rolling(fast).min().shift(1)
        dataframe["donchian_high_slow"] = high.rolling(slow).max().shift(1)
        dataframe["donchian_low_slow"] = low.rolling(slow).min().shift(1)

        dataframe["donchian_exit_high"] = high.rolling(ex).max().shift(1)
        dataframe["donchian_exit_low"] = low.rolling(ex).min().shift(1)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[short_break & vol_ok, ["enter_short", "enter_tag"]] = [1, "donchian_breakout_short"]
    ...
    def plot_config(self):
        return {
            "main_plot": {
                "close": {"color": "black"},
                "donchian_high_fast": {"color": "blue"},
                "donchian_low_fast": {"color": "blue"},
                "donchian_high_slow": {"color": "orange"},
                "donchian_low_slow": {"color": "orange"},
                "donchian_exit_high": {"color": "gray"},
                "donchian_exit_low": {"color": "gray"},
            },
            "subplots": {
                "ATR": {"atr": {"color": "white"}},
