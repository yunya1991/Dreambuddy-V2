...
class MacdTrendFollowingStrategy(IStrategy):
    INTERFACE_VERSION = 3
    ...
    macd_fast = IntParameter(6, 24, default=12, space="buy")
    macd_slow = IntParameter(18, 60, default=26, space="buy")
    macd_signal = IntParameter(5, 18, default=9, space="buy")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        prev_sig = dataframe["macdsignal"].shift(1)

        cross_up = (dataframe["macd"] > dataframe["macdsignal"]) & (prev_macd <= prev_sig)
        cross_dn = (dataframe["macd"] < dataframe["macdsignal"]) & (prev_macd >= prev_sig)

        vol_ok = dataframe["volume"] > 0
        dataframe.loc[cross_up & vol_ok, ["enter_long", "enter_tag"]] = [1, "macd_cross_up"]
        dataframe.loc[cross_dn & vol_ok, ["enter_short", "enter_tag"]] = [1, "macd_cross_down"]
    ...
    def plot_config(self):
        return {
            ...
            "subplots": {
                "MACD": {"macd": {"color": "blue"}, "macdsignal": {"color": "orange"}, "macdhist": {"color": "gray", "type": "bar"}},
                "ATR": {"atr": {"color": "white"}},
