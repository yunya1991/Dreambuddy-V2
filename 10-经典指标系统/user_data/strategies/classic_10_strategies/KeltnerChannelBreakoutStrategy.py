...
class KeltnerChannelBreakoutStrategy(IStrategy):
    ...
    ema_period = IntParameter(10, 80, default=20, space="buy")
    atr_period = IntParameter(7, 40, default=14, space="buy")
    channel_atr_mult = DecimalParameter(1.0, 4.0, default=2.0, space="buy")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ema_n = int(self.ema_period.value)
        atr_n = int(self.atr_period.value)
        mult = float(self.channel_atr_mult.value)

        dataframe["kc_middle"] = ta.EMA(dataframe, timeperiod=ema_n)
        dataframe["kc_atr"] = ta.ATR(dataframe, timeperiod=atr_n)
        dataframe["kc_upper"] = dataframe["kc_middle"] + dataframe["kc_atr"] * mult
        dataframe["kc_lower"] = dataframe["kc_middle"] - dataframe["kc_atr"] * mult
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        vol_ok = dataframe["volume"] > 0
        dataframe.loc[(dataframe["close"] <= dataframe["kc_middle"]) & vol_ok, "exit_long"] = 1
