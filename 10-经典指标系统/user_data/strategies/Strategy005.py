    ...
    def bot_start(self, **kwargs) -> None:
        ...
        try:
            ...
            if isinstance(cfg, dict):
                raw_cfg = cfg.get("strategy_params")
                if raw_cfg is None:
                    raw_cfg = cfg.get("strategy_parameters")
        except Exception:
            raw_cfg = None
        ...
        try:
            if isinstance(raw_cfg, dict):
                if "Strategy005" in raw_cfg and isinstance(raw_cfg.get("Strategy005"), dict):
                    params_cfg = dict(raw_cfg.get("Strategy005") or {})
                else:
                    params_cfg = dict(raw_cfg)
        except Exception:
            params_cfg = {}

        params_env: Dict[str, Any] = {}
        try:
            raw_env = str(os.environ.get("FT_STRATEGY_PARAMS_JSON") or "").strip()
            if raw_env:
                obj = json.loads(raw_env)
                if isinstance(obj, dict):
                    if "Strategy005" in obj and isinstance(obj.get("Strategy005"), dict):
                        params_env = dict(obj.get("Strategy005") or {})
                    else:
                        params_env = dict(obj)
        except Exception:
            params_env = {}
            ...
            if feature_url or decision_url:
                ...
                if pair:
                    ...
                    if not sig_df.empty:
                        ...
                        try:
                            if tf_ms > 0 and "date" in sig_df.columns:
                                dt_ms = pd.to_datetime(sig_df["date"], utc=True, errors="coerce").astype("int64") // 1_000_000
                                max_age_ms = int(max(int(2 * tf_ms), int(10 * 60 * 1000)))
                                sig_df = sig_df.loc[(dt_ms.notna()) & ((dt_ms + int(tf_ms)) >= int(now_ms - max_age_ms))]
                                if not sig_df.empty:
                                    sig_df = sig_df.sort_values("date").tail(3)
                            elif ("date" in sig_df.columns) and (not sig_df.empty):
                                sig_df = sig_df.sort_values("date").tail(1)
                        except Exception:
                            pass
