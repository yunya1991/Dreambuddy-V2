...
- 默认只做多模式
"""
import json, os, time, requests, warnings, math
...
def get_ma200_stop_loss(direction: str, close: float, daily_ma200: Optional[float], weekly_ma200: Optional[float]) -> Tuple[Optional[float], bool]:
    """
    MA200动态止损逻辑（适用于所有币种）
    返回: (止损线价格, 是否触发止损)
    ...
    if direction == "SHORT":
        # 硬性条件: 收盘价 > 日线MA200 → 止损
        if daily_ma200 is not None and close > daily_ma200:
            return daily_ma200, True

        # 动态止损线: 选择上方更近的MA200
        candidates = []
        if daily_ma200 is not None and daily_ma200 > close:
            candidates.append((daily_ma200, daily_ma200 - close))
        if weekly_ma200 is not None and weekly_ma200 > close:
            candidates.append((weekly_ma200, weekly_ma200 - close))

        if candidates:
            stop_line = min(candidates, key=lambda x: x[1])[0]
            return stop_line, close >= stop_line
        return None, False
        ...
        candidates = []
        if daily_ma200 is not None and daily_ma200 < close:
            candidates.append((daily_ma200, close - daily_ma200))
        if weekly_ma200 is not None and weekly_ma200 < close:
            candidates.append((weekly_ma200, close - weekly_ma200))
...
def v15_decision(prices: List[float]) -> dict:
    """V15策略决策函数"""
    if len(prices) < 30:
        ...
        if position is None:
            ...
            if not hit_tp and not hit_sl and daily_ma200_list is not None:
                ...
                if daily_ma200 is not None or weekly_ma200 is not None:
                    stop_line, triggered = get_ma200_stop_loss(direction, current_price, daily_ma200, weekly_ma200)
                    if triggered and stop_line is not None:
                        hit_sl = True
            ...
            if hit_tp or hit_sl:
                ...
                trade = {
                    ...
                    "exit_reason": "take_profit" if hit_tp else "ma200_stop",
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V15策略回测引擎 v3")
