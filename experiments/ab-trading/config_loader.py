...
def load_config(strategy_type: str = "v15ct") -> dict:
    """
    加载配置
    :param strategy_type: "v15ct" 或 "screen"
    :return: 合并后的配置字典
    """
    config_dir = Path(__file__).parent / "config"
    common_path = config_dir / ".env.common"
    
    if strategy_type == "v15ct":
        strategy_path = config_dir / ".env.v15ct"
    elif strategy_type == "screen":
        strategy_path = config_dir / ".env.screen"
    else:
...
if __name__ == "__main__":
    config = load_config("v15ct")
    print("=== V15-CT 配置 ===")
    for k, v in sorted(config.items()):
        print(f"{k}={v}")
    
    config = load_config("screen")
    print("\n=== 三屏马丁配置 ===")
    for k, v in sorted(config.items()):
        print(f"{k}={v}")