...
def gen_below_all_with_fib_zone(drift=-0.004, vol=0.015):
    ...
    from screen_executor import _calc_fibonacci, _calc_sma
...
def run_v15_with_prices(prices, current_price=None):
    ...
    screen1 = {"spot_inst": "BTC-USDT"}
    screen2 = {}
    with patch("screen_engine._fetch_candles", return_value=candles):
        return _v15_real_decision(screen1, screen2, price=current_price)


def get_indicators(prices, current_price=None):
    """计算 V15 内部指标"""
    from screen_executor import _calc_sma, _calc_rsi, _determine_position, _calc_fibonacci
    cp = current_price or prices[-1]
    smas = {p: _calc_sma(prices, p) for p in [30, 65, 128, 200]}
    rsi = _calc_rsi(prices, 14)
    pos = _determine_position(cp, smas)
    fib = _calc_fibonacci(prices, 30)
...
def test_fib_boundary_exact():
    ...
    for label, target in [("f382", fib['f382']), ("f500", fib['f500']), ("f618", fib['f618'])]:
        if target < min_sma:
            ...
            assert_true(f"Boundary {label}: no crash", result["mode"] == "v15_real")
...
def test_empty_candles():
    ...
    from screen_executor import _v15_real_decision
    with patch("screen_engine._fetch_candles", return_value=[]):
    ...
    assert_eq("Empty: mode", result["mode"], "v15_real")
...
def test_insufficient_data():
    ...
    assert_true("Insufficient: no crash", result["mode"] == "v15_real")
...
def test_extreme_volatility():
    ...
    result = run_v15_with_prices(prices)
    assert_true("Extreme vol: no crash", result["mode"] == "v15_real")
...
def test_sharp_drop():
    ...
    result = run_v15_with_prices(prices)
    assert_true("Sharp drop: no crash", result["mode"] == "v15_real")
...
def test_sharp_pump():
    ...
    result = run_v15_with_prices(prices)
    assert_true("Sharp pump: no crash", result["mode"] == "v15_real")
...
def test_v_shape_recovery():
    ...
    result = run_v15_with_prices(prices)
    assert_true("V-shape: no crash", result["mode"] == "v15_real")
...
def test_all_same_price():
    ...
    assert_true("Same price: no crash", result["mode"] == "v15_real")
...
def test_constant_decline():
    ...
    assert_true("Constant decline: no crash", result["mode"] == "v15_real")
...
def test_dual_mode_v15_ai_wait():
    """场景22: 对照模式 - AI返回WAIT，V15有信号"""
    from screen_executor import _dual_mode_decision

    prices, golden_price, _ = gen_below_all_with_fib_zone()
    candles = make_candles(prices)

    screen1 = {"spot_inst": "BTC-USDT"}
    reports = {"weekly": {}, "a1_daily": {}, "a6_intel": {}}
    ai_result = {"action": "WAIT", "confidence": 30, "reasons": ["AI观望"], "mode": "deepseek", "vol_mult": 1.0}

    with patch("screen_engine._fetch_candles", return_value=candles):
        with patch("screen_executor._llm_decision_deepseek", return_value=ai_result):
            with patch("screen_executor._load_quota", return_value={"deepseek": 0}):
                with patch("screen_executor.DEEPSEEK_API_KEY", "fake_key"):
                    with patch("screen_executor._record_usage"):
                        result = _dual_mode_decision(screen1, {}, {}, reports)

    v15_action = result.get("v15_real_result", {}).get("action", "WAIT")
    if v15_action != "WAIT":
        assert_eq("Dual AI-WAIT: mode", result["mode"], "dual_v15")
        assert_in("Dual AI-WAIT: reason", " ".join(result.get("reasons", [])), "V15")
    else:
        assert_eq("Dual both-WAIT: mode", result["mode"], "dual_wait")


def test_dual_mode_ai_signal():
    """场景23: 对照模式 - AI有入场信号"""
    from screen_executor import _dual_mode_decision
    ...
    screen1 = {"spot_inst": "BTC-USDT"}
    reports = {"weekly": {}, "a1_daily": {}, "a6_intel": {}}
    ai_result = {"action": "OPEN_BEAR", "confidence": 80, "reasons": ["AI看空"], "mode": "deepseek", "vol_mult": 1.5}
    ...
    assert_eq("Dual AI-signal: mode", result["mode"], "dual_ai")
    assert_eq("Dual AI-signal: action", result["action"], "OPEN_BEAR")
    assert_true("Dual AI-signal: has v15_real_result", "v15_real_result" in result)


def test_dual_mode_both_wait():
    """场景24: 对照模式 - AI和V15都WAIT（使用震荡中性数据）"""
    from screen_executor import _dual_mode_decision

    # 构造确保 V15 也返回 WAIT 的数据：震荡 + RSI中性
    random.seed(77)
    prices = gen_sideways(n=210, center=50000, vol=0.005)  # 低波动率
    ...
    screen1 = {"spot_inst": "BTC-USDT"}
    reports = {"weekly": {}, "a1_daily": {}, "a6_intel": {}}
    ai_result = {"action": "WAIT", "confidence": 20, "reasons": ["AI观望"], "mode": "deepseek", "vol_mult": 1.0}
    ...
    v15_action = result.get("v15_real_result", {}).get("action", "UNKNOWN")
    if v15_action == "WAIT":
        assert_eq("Dual both-WAIT: action", result["action"], "WAIT")
        assert_eq("Dual both-WAIT: mode", result["mode"], "dual_wait")
    else:
        # V15 有信号时使用 V15 入场
        assert_eq("Dual V15-fallback: mode", result["mode"], "dual_v15")
        assert_true("Dual V15-fallback: action not WAIT", result["action"] != "WAIT")


def test_dual_mode_ai_unavailable():
    """场景25: 对照模式 - AI不可用"""
    from screen_executor import _dual_mode_decision
    ...
    with patch("screen_engine._fetch_candles", return_value=candles):
        with patch("screen_executor.DEEPSEEK_API_KEY", ""):
            with patch("screen_executor._record_usage"):
                result = _dual_mode_decision(screen1, {}, {}, reports)

    assert_eq("Dual AI-unavailable: mode", result["mode"], "dual_v15_only")
    assert_true("Dual AI-unavailable: has v15_real_result", "v15_real_result" in result)
...
def main():
    ...
    tests = [
        ...
        # 对照模式
        ("对照: AI WAIT + V15有信号", test_dual_mode_v15_ai_wait),
        ("对照: AI有入场信号", test_dual_mode_ai_signal),
        ("对照: 双方都WAIT", test_dual_mode_both_wait),
        ("对照: AI不可用", test_dual_mode_ai_unavailable),
