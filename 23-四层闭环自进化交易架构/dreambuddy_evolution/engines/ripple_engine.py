...
class RippleEngine:
    ...
    def __init__(self, vol_ratio_threshold: float = 2.0, liq_change_threshold: float = 0.30):
        """
        龙头检测门槛（可配置）:
        - vol_ratio_threshold: vol_5/vol_20 放量倍数，默认 2.0（严格龙头）
          训练期可降至 1.3 以积累样本
        - liq_change_threshold: 清算指数变化率，默认 0.30
        """
        self._vol_ratio_threshold = float(vol_ratio_threshold)
        self._liq_change_threshold = float(liq_change_threshold)

    def detect_ripple_source(self, source: dict[str, Any]) -> bool:
        """
        §0.7.2 Phase 0: 龙头检测
        条件: S_up簇(ESS_top方向一致) + vol_5/vol_20≥门槛 + liq_index↑≥门槛 + Scale∈{Classical,Meso}
        """
        try:
            ...
            # 2. 放量: vol_5/vol_20 ≥ 门槛
            vol_ratio = vol_5 / max(vol_20, 1e-9)
            if vol_ratio < self._vol_ratio_threshold:
                return False

            # 3. 清算指数上升 ≥ 门槛
            if liq_change < self._liq_change_threshold:
                return False
