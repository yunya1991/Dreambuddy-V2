...
新增新闻源：
1. 金十数据（宏观快讯）
2. Twitter 加密大 V（分 4 类）
   - 技术派：Vitalik 等核心开发者
   - 投资机构：a16z, Tom Lee 等
   - 链上分析师：Willy Woo, PlanB 等
   - 交易员/创始人：Arthur Hayes 等
...
# 金十数据分类
JIN10_CATEGORIES = {
    "finance": {"weight": 1.2, "event_type": EventType.MARKET_ANALYSIS.value},
    "crypto": {"weight": 1.3, "event_type": EventType.JIN10_NEWS.value},
    "forex": {"weight": 1.1, "event_type": EventType.JIN10_NEWS.value},
    "metal": {"weight": 1.0, "event_type": EventType.JIN10_NEWS.value},
    "energy": {"weight": 1.0, "event_type": EventType.JIN10_NEWS.value},
}
...
class EnhancedEventLedgerGenerator:
    ...
    def generate_event_id(self) -> str:
        ...
        now = datetime.now()
        return f"EVT-V94-{now.strftime('%Y%m%d%H%M%S')}-{self.event_counter:04d}"
    ...
    def calculate_influencer_weight(self, influencer: str) -> tuple:
        ...
        info = self.get_influencer_info(influencer)
        tier_weight = {
            InfluencerTier.TIER_0.value: 1.5,
            InfluencerTier.TIER_1.value: 1.3,
            InfluencerTier.TIER_2.value: 1.1,
    ...
    def create_ledger_entry(self, news_item: Dict) -> EventLedgerEntry:
        ...
        entry = EventLedgerEntry(
            ...
            source_reliability=influencer_weight / 1.5,  # 归一化到 0-1
...
def generate_enhanced_mock_news(days: int = 90, prices: Dict = None) -> List[Dict]:
    """
    生成增强版模拟新闻（包含金十数据 + Twitter 大 V）
    """
    random.seed(42)
    now = datetime.now()
    news_list = []
    ...
    for date in dates:
        if date.weekday() >= 5:  # 周末减少新闻
            num_jin10 = random.randint(1, 3)
            num_twitter = random.randint(0, 2)
        else:
            num_jin10 = random.randint(2, 5)
            num_twitter = random.randint(1, 4)
...
def main():
    ...
    # 生成新闻
    news_list = generate_enhanced_mock_news(args.days, prices)
    ...
    if args.output:
        ...
        output_path = output_dir / f"event_ledger_v94_{ts}.jsonl"
