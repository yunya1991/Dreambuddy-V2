/**
 * DreamBuddy 基本面分析 Cordis plugin — Phase 1
 *
 * 通过 IPC 调用 Python server 的 fundamental_analysis 方法，
 * Python server 再通过 HTTP 调用 9-基本面分析 (port 3456 @ 49.233.123.96)。
 *
 * V1-1: 并行化验证 — 与 dreambuddy-indicators 并行调用
 * V1-2: 降级链验证 — 3456 不可达时 FAIL-OPEN 返回中性默认值
 */
...
const name = "dreambuddy-fundamental";
const inject = ["tools"];
...
function apply(ctx, config = {}) {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        "获取基本面分析数据（新闻情绪、资金流向、宏观指标、链上数据等）。数据来源：9-基本面分析 (port 3456 @ 49.233.123.96)。服务不可达时返回中性默认值。",
      parameters: {
        symbol: {
          type: "string",
          required: true,
        ...
        endpoint: {
          type: "string",
          description: "自定义 API endpoint 路径（默认 /fundamental/overview）",
        },
      },
      output: {
        schema: {
      ...
      async execute(args, _exec) {
        ...
        const response = await ipcClient.callMethod("fundamental_analysis", {
          symbol: args.symbol,
          endpoint: args.endpoint || "/fundamental/overview",
        });

        if (!response.ok) {
          throw new Error(
            `基本面分析调用失败: ${JSON.stringify(response.error || response.result)}`
          );
        }
      ...
      presentCall: (args) => ({
        card: "generic",
        title: `基本面: ${args.symbol}`,
        kind: "execute",
        rawInput: `${args.symbol}`,
        content: [
          {
            type: "text",
            text: `查询 ${args.symbol} 基本面分析`,
          },
        ],
      }),
      presentResult: (_args, result) => ({
        ...
        content: [
          {
            type: "text",
            text: `状态: ${result.status || "ok"} | 节点: ${result.node_id || "fundamental_analysis"}`,
          },
        ],
      }),
    })
