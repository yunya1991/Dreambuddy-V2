...
> **澄清说明（A 层 vs C 层动态图行为职责）**：
> 上表中"图编排执行：支持条件跳转、反思、动态调整"涉及 A 层与 C 层的协同，需明确分工：
> - **A 层（GraphPlanner）** 在主流程中产出的是 **SequentialGraph**（固定链路 + 场景扩展节点），即按意图和场景拼接出的顺序执行图。`ConditionalGraph` 虽然存在于代码中，但**当前并未用于 TradingAgent / AutoTrader 的主流程**。
> - **条件跳转、反思、动态调整**（REDO / INSERT_BEFORE / JUMP_TO / EARLY_TERMINATE 等动作）并非由 A 层图结构决定，而是由 **C 层 `Reflector` 在运行时**基于每步节点结果与状态做出反射决策，再由 `GraphExecutor` 改写执行轨迹。
> - 因此可概括为：**A 层负责静态图编排（链路 + 场景扩展），C 层负责运行时动态行为（反思与跳转）**。
...
为验证 Dream OS 在各种市场环境下的稳定性与正确性，建立了基于 36 种市场场景的模拟压力测试体系。测试工具位于 `cli/multi_scenario_stress_test.py`。

#### 1.4.1 36 场景分类体系

三维分类模型（3 趋势 × 4 波动率 × 3 动量 = 36 种场景）：

| 维度 | 取值 | 说明 |
|------|------|------|
| 趋势方向 | BULL / BEAR / NEUTRAL | 价格 vs EMA20/EMA50 排列 + 24h 涨跌幅 |
| 波动率等级 | LOW / NORMAL / HIGH / EXTREME | 基于 ATR% 阈值（0.5%/1%/2%/4%） |
| 动量加速度 | ACCELERATING / DECELERATING / EXHAUSTION | 短期 vs 中期动量对比 + 衰竭检测 |

分类器实现位于 `core/sense/scenario_classifier.py`，复用 A6 趋势/波动率计算、C2 动量计算、screen_engine 衰竭检测逻辑。

#### 1.4.2 压力测试五类指标

| 测试项 | 说明 | 最新结果 |
|--------|------|----------|
| 场景分类准确性 | 验证 36 种场景的模拟数据能被正确分类 | 100% |
| 编排记忆覆盖率 | 验证 36 种场景在 OrchestrationMemory 中均有对应配置 | 100%（L0:10, L1:8, L2:18） |
| 图编排-执行一致性 | 验证计划节点与实际执行的顶层节点数一致 | 97.2% |
| 边缘场景稳定性 | 10 种边缘场景（零波动、极值 RSI、崩盘、反弹、空信号源、降级等） | 10/10 通过 |
| 全链路压测 | 108 轮随机场景混合压测 | 成功率 100%，吞吐 3800+ TPS |

**综合评分：99.31 / 100（A 级）**

#### 1.4.3 编排记忆三级回退

OrchestrationMemory 采用三级回退查询策略，确保任何场景都能得到编排结果：

- **L0 精确匹配**：场景 ID 完全匹配，有回测数据支撑，置信度最高
- **L1 趋势+波动率匹配**：同趋势同波动率的其他场景聚合结果
- **L2 趋势匹配**：同趋势的所有场景聚合结果
- **L3 默认**：默认 c_chain [C1, C2, C3]，无回测数据

记忆表由场景回测器（`core/memory/scenario_backtester.py`）持续更新，随回测数据积累逐步从 L3 向 L0 演进。
