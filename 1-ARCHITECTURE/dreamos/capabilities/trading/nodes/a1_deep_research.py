...
class A1DeepResearchNode(BaseNode):
    ...
    def execute_core(self, state: State) -> NodeResult:
        ...
        # ── 4. P2-2: 调用 A0 矛盾论分析 ────────────────────
        a0_result = self._invoke_a0(state)
        a0_direction = a0_result.get("main_direction", "HOLD")
        a0_confidence = a0_result.get("confidence", 0.5)
        a0_main_contradiction = a0_result.get("main_contradiction", "")

        # A0 方向与 A1 方向一致性校验
        a0_aligned = (a0_direction == direction) or (a0_direction == "HOLD")
        ...
        # P2-2: A0 矛盾论方向一致性调整
        # 仅保留正向激励：A0支持时加分，不支持时不惩罚（避免系统性打压confidence）
        ...
        # P2-2: A0 矛盾论结果
        rationale.append(f"  [A0矛盾论] 主矛盾: {a0_main_contradiction} → 方向{a0_direction} (置信{a0_confidence:.1%})")
        rationale.append(f"  [A0门禁] 方向{'一致' if a0_aligned else '矛盾'} → 置信{'加成+0.06' if a0_aligned and a0_direction != 'HOLD' else '无调整'}")
        ...
        return NodeResult(
            ...
            outputs={
                ...
                "a0_contradiction": a0_result,
                "a0_aligned": a0_aligned,
                "rationale": rationale,
            },
        )

    def _invoke_a0(self, state: State) -> Dict[str, Any]:
        """P2-2: 调用 A0 矛盾论节点进行主要矛盾分析"""
        try:
            from dreamos.capabilities.trading.nodes.a0_contradiction import A0ContradictionNode
            a0_node = A0ContradictionNode()
            a0_result = a0_node.execute_core(state)
            return {
                "main_contradiction": a0_result.outputs.get("main_contradiction", ""),
                "main_direction": a0_result.outputs.get("main_direction", "HOLD"),
                "intensity": a0_result.outputs.get("intensity", 0),
                "confidence": a0_result.confidence,
                "dimensions": a0_result.outputs.get("dimensions", {}),
            }
        except Exception as e:
            # A0 调用失败不阻断 A1，返回中性结果
            return {
                "main_contradiction": "",
                "main_direction": "HOLD",
                "intensity": 0,
