...
> **核心理念**: 思维链为骨架 + AI推理动态化 + 技能模块化 + 图结构可追溯
> **实现状态**: 🟢 已实现 / 🟡 部分实现 / 🔴 规划中
...
DreamBuddy v2 采用**"骨架-血肉-灵魂"**三层架构设计：

| 层级 | 类比 | 对应概念 | 作用 | 状态 |
|------|------|---------|------|------|
| **骨架层** | 人体骨骼 | 三大思维链（S/C/F） | 定义思考的顺序和框架，不定义具体内容 | 🟢 已实现 |
| **血肉层** | 器官组织 | SKILL模块化能力库 | 提供具体专业能力，可动态调用 | 🟢 已实现 |
| **灵魂层** | 大脑神经 | AI推理引擎 + 反思引擎 | 动态决策、路径选择、置信度评估 | 🟡 部分实现 |

**核心原则**：
- 思维链是**骨架**，不是实现——每步内部由AI动态决定调用什么技能
- 技能是**可插拔模块**——统一契约，独立优化，按链分类（A/C/F系列）
- 推理是**动态进化**的——置信度不足时追加节点，高置信时跳过
- 全过程**图结构记录**——可追溯、可压缩、可展开
...
> **重要：S 链 vs A 系列**
>
> - **S链** = 思维链骨架（S1-S5 五步框架），是"骨架"
> - **A系列技能** = S链的具体实现模块，是"血肉"
> - 关系：S链定义"走哪几步"，A系列技能定义"每步具体做什么"
> - 同理：C链有C系列技能，F链有F系列技能

```
S链（思维骨架）      A系列技能（血肉）
┌──────────┐      ┌──────────────────┐
│ S1 调研   │ ←──  │ dream-strategy-  │
│          │      │ research / 情报  │
├──────────┤      ├──────────────────┤
│ S2 分析   │ ←──  │ contradiction-  │
│          │      │ theory / 大师研讨 │
...
└───────────────────────────────┬─────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                  API 编排层 (/api/orchestrate) 🟡                          │
│                                                                         │
│  前端 ←→ 后端的桥梁                                                      │
│  接收请求 → 调用后端编排器 → 流式返回结果 → 更新前端展示                  │
│  用户反馈 → 触发重规划 → 调整执行路径                                     │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
...
│                                                                         │
│  用户输入 → 意图分类 → 推荐主链(S/C/F) → 扩展节点池 → 上下文注入         │
│  零Token · 本地计算 · 基于市场数据+记忆+知识库                           │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                  链路规划层 (ChainPlanner) ★ 零Token 🟡                    │
│                                                                         │
│  【A阶段规划 — 形成初步执行图】                                           │
│                                                                         │
│  四维规划：                                                              │
│  1. Token预算过滤 → 剪掉超预算节点                                       │
│  2. 知识库命中提升 → 高分策略走快捷路径                                   │
│  3. 历史表现过滤 → 当前Regime+标的命中率                                 │
│  4. 标的覆盖检查 → 冷门标的标记风险节点                                   │
│                                                                         │
│  输出：PlanResult（最优节点序列 + 剪枝记录 + 规划理由）                  │
│  同时构建：B层蓝图（总目标）+ A层初始执行图                               │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│               图结构架构层 (Graph Architecture — B层) 🟢                   │
│                                                                         │
│  🏗️ Blueprint（蓝图/总目标）—— 一般不变                                   │
│    • 定义系统组件和数据流向                                              │
│    • 模块级粒度，确定本次任务的总体框架                                   │
│    • 对应 OKR 的 Long-term Objective                                    │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│               动态执行层 (Dynamic Execution — A层可变) 🟡                  │
│                                                                         │
│  🔀 Architecture（DAG执行图）—— AI驱动，动态变化                          │
│                                                                         │
│  主循环：generateInitialPlan → executeStep → reflect → 调整 → 下一步     │
│                                                                         │
│  反思决策类型（5种）：                                                   │
│    • CONTINUE         — 正常推进                                         │
│    • REDO             — 置信度过低/风险过高，重做当前步                   │
│    • INSERT_BEFORE    — 缺少信息，插入补充节点                           │
│    • JUMP_TO          — 高置信，跳过后续验证直接执行                      │
│    • EARLY_TERMINATE  — 基本完成，提前终止                               │
│                                                                         │
│  【两阶段三链结合策略】                                                   │
│  阶段一：简单问题 → 交叉验证投票（A/C/F三链加权投票）🟢                    │
│  阶段二：复杂问题 → 动态插入节点（从技能注册表追加其他链步骤）🟡            │
│                                                                         │
│  节点变化来源：                                                          │
│    1. 三链SKILL注册表动态添加（从A/C/F技能池选择）                       │
│    2. AI自行思考追加能力（推理决定需要什么）🟡                             │
│    3. 联网搜索/工具调用补充（技能不够时扩展）🔴                            │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│               执行记录层 (Execution Chronicle — C层) 🟢                    │
│                                                                         │
│  ⏱️ Chronicle（时间线）—— 实际执行记录                                    │
│    • 每步完整输出 + 置信度 + 耗时 + Token成本                            │
│    • 完整决策轨迹：为什么选这个技能？为什么跳/重做？                      │
│    • 可回溯、可复盘、可压缩                                              │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
...
│  调研报告 / 策略方案 / 回测报告 / 执行日志 / 离场报告                    │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│              记忆与自我进化层 (Memory & Evolution) ★ 动力引擎 🟡            │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  🔍 发现问题层                                                    │    │
│  │    执行失败/置信度低/三链分歧/用户反馈/风控触发                   │    │
│  └─────────────────────────────┬───────────────────────────────────┘    │
│                                │                                        │
│                                ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  📝 学习记录层                                                    │    │
│  │    Episode Writer → Lesson Distiller → 规律提炼                  │    │
│  │    （每轮决策都记录，skip也写，作为事实底座）                     │    │
│  └─────────────────────────────┬───────────────────────────────────┘    │
│                                │                                        │
│                                ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  🌙 深度分析层（做梦部+治理闭环）                                  │    │
│  │    做梦部：潜意识分析/反事实推演/被压制判断                       │    │
│  │    A7实践论：实践→认识→实践 循环                                 │    │
│  │    A8知行合一：纯粹理性内部批评自循环                             │    │
│  └─────────────────────────────┬───────────────────────────────────┘    │
│                                │                                        │
...
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  🌐 协作网络层（Dream-Agent）                                     │    │
│  │    多Agent协作 + 账本系统 + DREAM Token                           │    │
│  │    飞书审批 + GitHub门禁 + 去中心化激励                             │    │
│  │    （大规模协作网络，实现规模效应）                                 │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                         │
│  三大基础能力：                                                          │
│  📚 知识库 (Knowledge Base)  — 策略/方法论/理论/运营知识                 │
│  🧠 记忆系统 (Memory System) — 意图记忆/用户偏好/经验模式                │
│  📇 索引系统 (Index System)  — L1主索引/L2目录索引/L3文件锚点            │
└─────────────────────────────────────────────────────────────────────────┘
```
