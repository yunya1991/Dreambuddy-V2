...
DreamBuddy v2 采用**"三层价值模型 + 图架构压缩"**的操作系统级架构设计：

| 层级 | 类比 | 对应概念 | 作用 | 状态 |
|------|------|---------|------|------|
| **S层 (意图识别层)** | 操作系统用户态 | Objective → OKR → Blueprint | 理解用户意图，转化为工程化目标 | 🟢 已实现 |
| **A层 (图编排层)** | 操作系统调度器 | GraphOrchestrator + NodeRegistry | 动态构建执行图，调度节点执行 | 🟢 已实现 |
| **C层 (执行层)** | 操作系统内核态 | UnifiedExecutor + 动态链融合 | 具体节点执行 + 动态重规划 + 反思进化 | 🟢 已实现 |
| **G层 (图存储压缩层)** | 操作系统文件系统 | G.B / G.A / G.C 三层模型 | 执行记录存储 + 正向展开 + 回溯压缩 | 🟢 已实现 |

**核心原则**：
- **用户意图驱动**：从自然语言到 OKR 再到工程化蓝图，三层递进
...
| 架构层 | 定位 | 包含的技能系列 | 典型模块 |
|-------|------|--------------|---------|
| **S层** (意图识别层) | 用户态，理解意图 | （S层内部模块） | IntentGateway、ChainPlanner |
| **A层** (图编排层) | 调度器，构建执行图 | A系列（AI交易技能） | GraphOrchestrator、NodeRegistry、A0-A9技能 |
| **C层** (执行层) | 内核态，执行节点 | C系列（经典量化）、F系列（基本面） | UnifiedExecutor、动态链融合、C1-C5、F1-F5 |
| **G层** (图存储压缩层) | 文件系统，存储追溯 | G系列（治理支持） | G.B / G.A / G.C 三层模型、Compressor |

```
┌─────────────────────────────────────────────────────────────┐
│                    S层 (意图识别层)                           │
│  Objective → OKR → Blueprint                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐          │
│  │ IntentGateway│  │ ChainPlanner │  │ 上下文注入   │          │
│  └─────────────┘  └─────────────┘  └─────────────┘          │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    A层 (图编排层)                             │
│  GraphOrchestrator + NodeRegistry                           │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  A系列技能（S链实现）                                  │    │
│  │  A0矛盾→A1调研→A2分析→A3设计→A4验证→A5执行→A6监控→A7门禁 │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
...
DreamBuddy v2.2 采用**七层架构**设计，自上而下分别为：前端层、API层、S层（意图识别层）、A层（图编排层）、C层（执行层）、G层（图存储压缩层）、基础设施层。

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    前端层 (Frontend — 展示+交互) 🟢                       │
│  交易Dashboard / 策略管理 / 配置 / 飞书集成 / 产物展示                    │
│  • 接收用户输入                                                           │
│  • 展示执行过程和结果                                                     │
│  • 用户反馈触发调整（双向机制）                                           │
└───────────────────────────────┬─────────────────────────────────────────┘
...
│  Objective → OKR → Blueprint                                            │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐          │
│  │ IntentGateway   │  │ ChainPlanner    │  │ 上下文注入器     │          │
│  │ 意图分类/推荐    │  │ 四维规划/剪枝    │  │ 记忆/知识库注入  │          │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘          │
...
│  ┌─────────────────────────────────────────────────────────────┐        │
│  │  A系列技能（AI交易技能 — S链实现）                             │        │
│  │  A0矛盾→A1调研→A2分析→A3设计→A4验证→A5执行→A6监控→A7门禁→A9离场 │        │
│  └─────────────────────────────────────────────────────────────┘        │
...
│  ┌─────────┐  ┌─────────┐  ┌─────────┐                                  │
│  │ G.B 蓝图 │  │ G.A 执行图│  │ G.C 记录 │                                  │
│  │ (Blueprint)│ │ (Architecture)│ │ (Chronicle)│                        │
│  └─────────┘  └─────────┘  └─────────┘                                  │
│  正向展开 (B→A→C) · 回溯压缩 (C→A→B) · 增量压缩 · 语义压缩                │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐                 │
│  │ 产物中台  │  │ 记忆系统  │  │ 知识库    │  │ 索引系统  │                 │
│  │ Artifact │  │ Memory   │  │ Knowledge│  │ Index    │                 │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘                 │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                               │
│  │ 自我进化  │  │ 治理系统  │  │ 数据服务  │                               │
│  │ Evolution│  │Governance│  │  Market   │                               │
│  └──────────┘  └──────────┘  └──────────┘                               │
└─────────────────────────────────────────────────────────────────────────┘
```

**数据流方向**：用户请求 → 前端 → API层 → S层（意图识别）→ A层（图编排）→ C层（执行）→ G层（存储压缩）→ 基础设施层

**控制流方向**：基础设施层支撑各层运行，G层记录执行过程，A层调度C层执行，S层定义目标和蓝图

---

### 1.4 核心组件清单（已实现）

本节列出系统各层的核心组件及其文件路径，便于快速定位和理解代码结构。
