#!/usr/bin/env python3
"""
V15 经典马丁策略信号模块（16层入场决策 + 16项技术指标）
基于斐波那契回调区 + 布林带均值回归 + RSI/MACD/ADX + 多指标共振的纯技术分析马丁策略

本模块独立于三屏系统，是 V15 经典马丁策略的核心信号引擎。
"""
import sys
...
def calc_supertrend(prices, period=10, multiplier=3.0):
    ...
    if len(prices) >= period + 1:
        ...
        prev_trend = (
            "BULL"
            if prev_price > (sum(prices[-period - 1 : -1]) / period - multiplier * atr)
            else "BEAR"
        )
    ...
    return {
        ...
        "atr": round(atr, 4),
        "distance_pct": (
            round(abs(price - lower_band) / lower_band * 100, 2) if lower_band > 0 else 0
        ),
    }
...
def v15_decision(
    ...
    volumes = [float(c.get("vol", 1)) for c in daily_candles]
    ...
    if position == "BELOW_ALL":
        reasons.append("价格在所有均线下方(BELOW_ALL)")
        ...
        if not short_enabled:
            ...
            if boll:
                reasons.append(
                    f"布林带: 上轨{boll['upper']:.4g} 中轨{boll['sma']:.4g} 下轨{boll['lower']:.4g}"
                )

            boll_near_mid_s = (
                boll and boll["sma"] > 0 and abs(current_price - boll["sma"]) / boll["sma"] < 0.02
            )
            ...
            if (
                in_zone_short
                and current_price >= f500_short
                and rsi > 45
                and (boll_near_mid_s or boll_touch_upper)
            ):
                fib_zone = "golden"
                ...
                size_mult = 1.0
                reasons.append(
                    f"Fib黄金区+布林{'上轨' if boll_touch_upper else '中轨'}双重确认做空, 仓位倍数{size_mult}"
                )
            ...
            # Tier 7: ADX强趋势 + -DI > +DI = 空头趋势确认
                ...
                size_mult = 0.7
                reasons.append(
                    f"ADX={adx['adx']}>25, -DI={adx['di_minus']}>+DI={adx['di_plus']}, 强空头趋势"
                )
            ...
            # Tier 9: OBV空头趋势 + 量能加速
                ...
                confidence = 66
                size_mult = 0.6
                reasons.append("OBV空头趋势加速, 资金流出确认SHORT")
            ...
            # Tier 11: Keltner Channel 上沿/中线
                ...
                size_mult = 0.5
                reasons.append(
                    f"Keltner Channel{'上沿' if keltner['near_upper'] else '中线'}入场(position={keltner['position']}), 均值回归SHORT"
                )
            ...
            # Tier 13: Vortex空头反转
                ...
                size_mult = 0.5
                reasons.append(
                    f"Vortex空头反转(VI-={vortex['vi_minus']}>VI+={vortex['vi_plus']}), 趋势反转确认SHORT"
                )
            ...
            # Tier 14: TEMA空头趋势
                ...
                size_mult = 0.5
                reasons.append(
                    f"TEMA空头趋势(tema={tema['tema']}<price, slope={tema['slope']}%), 三重EMA确认SHORT"
                )
            ...
            # Tier 15: GoldenCross死叉
                ...
                size_mult = 0.7
                reasons.append(
                    f"GoldenCross死叉(EMA50={golden_cross['ema_fast']}<EMA200={golden_cross['ema_slow']}), 长期趋势启动SHORT"
                )

            # Tier 16: EMA排列空头
            elif (
                ema_align
                and ema_align.get("direction") == "BEAR"
                and ema_align["aligned"]
                and rsi > 35
            ):
                ...
                size_mult = 0.8
                reasons.append(
                    f"EMA排列空头(EMA20<EMA50<EMA200, 对齐度={ema_align['alignment_score']}), 完美空头排列SHORT"
                )
        ...
        reasons.append("价格在所有均线上方(ABOVE_ALL)")
        reasons.append(f"Fib回调区: {f618_long:.4g}-{f382_long:.4g}, 当前价: {current_price:.4g}")
        reasons.append(f"RSI14: {rsi}")
        if boll:
            reasons.append(
                f"布林带: 上轨{boll['upper']:.4g} 中轨{boll['sma']:.4g} 下轨{boll['lower']:.4g}"
        ...
        if (
            ...
            size_mult = 1.0
            reasons.append(
                f"Fib黄金区+布林{'下轨' if boll_touch_lower else '中轨'}双重确认, 仓位倍数{size_mult}"
            )
        ...
        # Tier 5: RSI偏低（动能仍在多头侧）
            ...
            confidence = 60
            size_mult = 0.5
            reasons.append("RSI<45多头动能持续, 轻仓LONG")
        ...
        # Tier 7: ADX强趋势 + +DI > -DI = 多头趋势确认
            ...
            size_mult = 0.7
            reasons.append(
                f"ADX={adx['adx']}>25, +DI={adx['di_plus']}>-DI={adx['di_minus']}, 强多头趋势"
            )
        ...
        # Tier 9: OBV多头趋势 + 量能加速 = 资金流入确认
            ...
            size_mult = 0.6
            reasons.append(
                f"OBV多头趋势加速(OBV={obv['obv']} > MA={obv['obv_ma']}), 资金流入确认LONG"
            )

        # Tier 10: SuperTrend多头 = 趋势顺势信号
        ...
        # Tier 11: Keltner Channel 下沿/中线 = 均值回归机会
            ...
            size_mult = 0.5
            reasons.append(
                f"Keltner Channel{'下沿' if keltner['near_lower'] else '中线'}入场(position={keltner['position']}), 均值回归LONG"
            )
        ...
        # Tier 12: StochRSI金叉/超卖 = 动量反转信号
            ...
            size_mult = 0.5
            reasons.append(
                f"StochRSI{'金叉' if stochrsi['cross']=='golden' else '超卖'}(K={stochrsi['k']} D={stochrsi['d']}), 动量反转LONG"
            )

        # Tier 13: Vortex多头反转 = 趋势反转确认
            ...
            size_mult = 0.5
            reasons.append(
                f"Vortex多头反转(VI+={vortex['vi_plus']}>VI-={vortex['vi_minus']}), 趋势反转确认LONG"
            )

        # Tier 14: TEMA多头趋势 = 三重EMA确认
            ...
            size_mult = 0.5
            reasons.append(
                f"TEMA多头趋势(tema={tema['tema']}>price, slope={tema['slope']}%), 三重EMA确认LONG"
            )

        # Tier 15: GoldenCross金叉 = 长期趋势启动信号
            ...
            size_mult = 0.7
            reasons.append(
                f"GoldenCross金叉(EMA50={golden_cross['ema_fast']}>EMA200={golden_cross['ema_slow']}), 长期趋势启动LONG"
            )

        # Tier 16: EMA排列多头 = 均线完美对齐
            ...
            size_mult = 0.8
            reasons.append(
                f"EMA排列多头(EMA20>EMA50>EMA200, 对齐度={ema_align['alignment_score']}), 完美多头排列LONG"
            )
        ...
        if boll:
            reasons.append(
                f"布林带: 上轨{boll['upper']:.4g} 中轨{boll['sma']:.4g} 下轨{boll['lower']:.4g} 带宽{boll['bandwidth']}%"
            )
            ...
            if current_price <= boll["lower"] and rsi < 45:
                ...
                confidence = 64
                size_mult = 0.5
                reasons.append("OBV多头加速确认LONG")
            elif supertrend and supertrend["bullish"] and rsi < 60:
                trend_signal = "supertrend_bull"
                action = "OPEN_BULL"
                confidence = 62
                size_mult = 0.5
                reasons.append("SuperTrend多头趋势LONG")
                ...
                size_mult = 0.5
                reasons.append(
                    f"Vortex多头反转(VI+={vortex['vi_plus']}>VI-={vortex['vi_minus']}), 趋势反转LONG"
                )
            elif tema and tema["bullish"] and tema["slope"] > 0 and rsi < 65:
                trend_signal = "tema_bull"
                action = "OPEN_BULL"
                ...
                confidence = 70
                size_mult = 0.6
                reasons.append("GoldenCross金叉(EMA50>EMA200), 长期趋势启动LONG")
            elif ema_align and ema_align["bullish"] and ema_align["aligned"] and rsi < 65:
                trend_signal = "ema_align_bull"
                action = "OPEN_BULL"
                confidence = 72
                size_mult = 0.7
                reasons.append("EMA排列多头(EMA20>EMA50>EMA200), 完美多头排列LONG")
...
# 兼容旧接口名
v15_real_decision = v15_decision
