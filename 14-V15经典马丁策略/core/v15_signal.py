...
def v15_decision(spot_inst: str = "BTC-USDT", price: float = None,
    ...
    if not daily_candles:
        return {"action": "WAIT", "confidence": 0, "reasons": ["无法获取K线数据"], "mode": "v15", "vol_mult": 1.0}
    ...
    if current_price <= 0 or len(prices) < 30:
        return {"action": "WAIT", "confidence": 0, "reasons": ["价格数据异常"], "mode": "v15", "vol_mult": 1.0}
    ...
    if position == 'BELOW_ALL':
        ...
        if boll:
            reasons.append(f"布林带: 上轨{boll['upper']:.4g} 中轨{boll['sma']:.4g} 下轨{boll['lower']:.4g} 带宽{boll['bandwidth']}%")
