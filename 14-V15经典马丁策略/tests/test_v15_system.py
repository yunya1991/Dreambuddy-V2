...
def mock_candles(candles):
    """mock fetch_candles (market_data + v15_signal 两个绑定)"""
    import market_data
    import v15_signal
    original_md = market_data.fetch_candles
    original_v15 = v15_signal.fetch_candles
    ...
    return (original_md, original_v15)


def restore_candles(original):
    import market_data
    import v15_signal
    original_md, original_v15 = original
    market_data.fetch_candles = original_md
    v15_signal.fetch_candles = original_v15
...
def test_vol_mult_consistency():
    ...
    orig2 = mock_candles(candles2)
    r2 = v15_decision("BTC-USDT")
...
def test_integration_decision():
    ...
    assert_eq(result["mode"], "v15", "mode=v15")
...
def test_consistency_across_calls():
    print("\n" + "=" * 60)
    print("测试16: 相同输入 → 相同输出 (一致性)")
    print("=" * 60)

    candles = gen_synthetic_candles(60000, 200, "flat", 0.03)
    orig = mock_candles(candles)

    r1 = v15_decision("BTC-USDT")
    r2 = v15_decision("BTC-USDT")
...
def test_no_crash_extra_params():
    print("\n" + "=" * 60)
    print("测试18: 额外参数兼容性")
    print("=" * 60)
    ...
    # 默认调用
    r = v15_decision("BTC-USDT")
    assert_in(r["action"], ["OPEN_BULL", "OPEN_BEAR", "WAIT"], "默认调用 → 不崩溃")

    # 显式指定price
    r2 = v15_decision("BTC-USDT", 60000)
    assert_in(r2["action"], ["OPEN_BULL", "OPEN_BEAR", "WAIT"], "显式price → 不崩溃")

    # 显式指定全部参数
    r3 = v15_decision("BTC-USDT", 60000, "4H", 200)
    assert_in(r3["action"], ["OPEN_BULL", "OPEN_BEAR", "WAIT"], "全参数 → 不崩溃")
