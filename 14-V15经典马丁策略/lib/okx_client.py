#!/usr/bin/env python3
"""
OKX 交易客户端 - V15经典马丁策略专用
基于 OKX API，支持实盘/模拟盘交易、持仓跟踪、盈亏统计
...
CONFIG_DIR = Path(__file__).resolve().parent.parent / "data" / "okx_client"
...
def _load_config() -> Dict:
    ...
    if env_path.exists():
        ...
        leverage = cfg.get("default_leverage", 5.0)
...
class OKXSimulatedClient:
    ...
    def _usdt_to_sz(self, inst_id: str, usdt_amount: float) -> float:
        ...
        # SymbolMapper 合约参数覆盖（贵金属等 TradFi 资产）
        try:
            from symbol_mapper import from_swap as _from_swap, get_contract_overrides as _get_overrides
            coin = _from_swap(inst_id)
            if coin:
                overrides = _get_overrides(coin)
                if overrides.get("lot_sz") is not None:
                    lot_sz = overrides["lot_sz"]
                if overrides.get("ct_val") is not None:
                    ct_val = overrides["ct_val"]
        except Exception:
            pass
        ...
        aligned_sz = max(aligned_sz, lot_sz) if lot_sz > 0 else max(aligned_sz, 0.01)
