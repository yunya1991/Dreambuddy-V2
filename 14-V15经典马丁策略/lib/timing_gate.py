...
def retrace_quality_score(r: float, lo: float = 0.30, hi: float = 0.72,
                          mu: float = 0.5, sigma: float = 0.18) -> float:
    """
    Fib 回撤位质量评分（0~1）。
...
# 三浪结构识别（纯函数，便于单元测试）
# --------------------------------------------------------------------------- #
def detect_three_wave_structure(swings_raw: List[Any], fib_ext_ratio: float = 1.618) -> WaveStructure:
    """
    ...
    def _try_bull(triplet: List[Dict[str, float]]) -> Optional[WaveStructure]:
        ...
        return WaveStructure(
            kind="BULLISH_3WAVE",
            wave1_start=L0, wave1_end=H1, wave2_end=L2,
            wave1_range=wave1, retrace_ratio=r, ext_target=ext,
    ...
    def _try_bear(triplet: List[Dict[str, float]]) -> Optional[WaveStructure]:
        ...
        return WaveStructure(
            kind="BEARISH_3WAVE",
            wave1_start=H0, wave1_end=L1, wave2_end=H2,
            wave1_range=wave1, retrace_ratio=r, ext_target=ext,
            raw_swings_used=len(used),
        )
...
class TimingGate:
    ...
    def _eval_one_tf(
        self, closes: List[float], price_now: float,
        g_long: bool, g_short: bool, window: int = None,
    ) -> dict:
        ...
        if structure.kind == "UNCLEAR":
            ...
            retrace_score = retrace_quality_score(
                r,
                lo=self.fib_retrace_lo, hi=self.fib_retrace_hi,
                mu=self.retrace_mu, sigma=self.retrace_sigma,
            )
    ...
    def _extract_direction(gate_result: Any) -> tuple[bool, bool, str]:
        ...
        if isinstance(gate_result, dict):
            g_long = bool(gate_result.get("long_allowed", True) or gate_result.get("long_enabled", True))
            g_short = bool(gate_result.get("short_allowed", False) or gate_result.get("short_enabled", False))
    ...
    def evaluate(
        ...
        # ---- 日线结构评估 ----
        daily = self._eval_one_tf(recent_closes, price_now, g_long, g_short, window=self.swing_window)
        ...
        if use_intraday:
            intra = self._eval_one_tf(
                intraday_closes, price_now, g_long, g_short, window=self.intraday_swing_window,
            )
        ...
        if structure.kind == "UNCLEAR":
            ...
            parts.append(
                ("多头三浪(BULLISH_3WAVE)" if structure.kind == "BULLISH_3WAVE"
        ...
        parts.append(
            "score=%.2f = struct×%.2f × retrace×%.2f × ext×%.2f" % (
                timing_score, structure_score, retrace_score, ext_score,
            )
        )
