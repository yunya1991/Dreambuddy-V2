...
独立于三屏马丁的实盘执行器，使用 trend-system/.env 配置
支持实盘下单（DRY_RUN=false），每60秒轮询一次

核心逻辑：
1. 调用 compute_full_trading_signal 获取 V4+波浪融合信号
2. 根据信号执行开仓/平仓/加仓操作
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(Path(__file__).parent.parent / "logs" / "v4_wave_trader.log"),
...
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "10-经典指标系统"))
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "11-易经推理系统"))
...
# ── 止盈止损策略融合参数 ──────────────────────────────────────────────────
# 基于 V4 趋势方向 + 波浪浪型 + 物理置信度，动态生成 SL/TP
# 设计原则：
#   - 趋势延续型（W1/W3/IMPULSE）：宽止损 + 移动止盈，让利润奔跑
#   - 趋势反转型（W5/CORRECTIVE）：窄止损 + 固定目标，快速落袋
#   - 波浪置信度低：收紧止损保护
#   - 物理置信度低：收紧止损保护

# 止损参数（按入场价的百分比）
SL_BASE_PCT = 0.04              # 基础止损 4%
SL_TREND_CONTINUATION_PCT = 0.06  # 趋势延续型止损 6%（给波动空间）
SL_REVERSAL_PCT = 0.025         # 反转型止损 2.5%（快速认错）
SL_LOW_CONFIDENCE_MULT = 0.7    # 低置信度止损收紧系数

# 止盈参数（按入场价的百分比）
TP_TREND_CONTINUATION_PCT = 0.15  # 趋势延续型目标 15%
TP_REVERSAL_PCT = 0.06           # 反转型目标 6%
TP_TRAILING_ACTIVATE_PCT = 0.05  # 移动止盈激活阈值 5%（盈利达此值启动追踪）
TP_TRAILING_CALLBACK_PCT = 0.025  # 移动止盈回撤 2.5%（从最高点回撤此值平仓）

# 波浪浪型分类
WAVE_TREND_CONTINUATION = {"IMPULSE_5", "W1", "W3", "W5"}
WAVE_REVERSAL = {"CORRECTIVE_3", "W2", "W4", "ABC"}
WAVE_LOW_CONFIDENCE_THRESHOLD = 0.5  # 波浪置信度 < 0.5 视为低置信度
...
def _compute_sltp(full_signal: dict, current_price: float) -> dict:
    ...
    Returns:
        {
            "stop_loss_pct": float,      # 止损百分比（0.04 = 4%）
            "take_profit_pct": float,    # 止盈百分比（0.15 = 15%）
            "trailing_enabled": bool,    # 是否启用移动止盈
    ...
    if is_reversal:
        ...
        sl_pct = SL_BASE_PCT
        tp_pct = TP_TREND_CONTINUATION_PCT * 0.6  # 默认取趋势目标的 60%
        trailing_enabled = False
        mode = "default"
...
class V4WaveTrader:
    def __init__(self):
        ...
        # 持仓级 SL/TP 跟踪表：{symbol: {entry_px, side, sl_pct, tp_pct, trailing, peak_px, ...}}
        self._position_sltp = {}
        self._position_meta_path = Path(__file__).parent.parent / "data" / "v4_position_sltp.json"
        self._load_position_meta()

    def _load_position_meta(self):
        """从磁盘加载持仓 SL/TP 元数据（跨重启持久化）"""
        try:
    ...
    def run_once(self):
        ...
        # ── 补全已有持仓的 qty + 补挂 SL/TP 硬单（重启后/升级后兼容）──
        for sym, pos_data in positions.items():
            ...
            # 补挂硬单（没有 sl_order_id 且非 dry_run）
        ...
        for symbol in TREND_SYMBOLS:
            ...
            try:
                ...
                full_signal = compute_full_trading_signal(spot_inst, is_btc=(symbol_upper == "BTC"))
    ...
    def _check_sltp(self, positions: dict):
        ...
        for symbol, pos_data in positions.items():
            ...
            if not meta:
                # 无元数据的持仓（可能是手动开的或重启前遗留），跳过 SL/TP 检查
                continue
            ...
            if trailing_enabled and pnl_pct > 0:
                ...
                # 移动止盈触发：盈利达激活阈值 + 从峰值回撤超 callback
            ...
            # 固定止损检查（支持锁定利润场景：sl_pct <= 0 表示 pnl 低于该正阈值时止损）
    ...
    def _dynamic_adjust_sltp(self, positions: dict, full_signal_map: dict):
        """动态评估调整 SL/TP（每轮基于最新信号）

        调整原则：
        - 风险升高（波浪置信度下降/浪型转反转/趋势方向反转）：只收不松
            - SL 收紧（向入场价靠近）
            - TP 不变或降低
            - 关闭移动止盈
        - 趋势增强（波浪置信度上升/浪型延续/方向一致且增强）：可放松
            - TP 可上调（让利润奔跑）
            - SL 保持不变（不放宽止损）
            - 保持移动止盈
        - 方向冲突或中性：维持现状
        ...
        for symbol, pos_data in positions.items():
            ...
            # 浪型从反转/默认转为趋势延续（old_mode 不含 trend_continuation 即视为非延续）
            ...
            if risk_increasing:
                ...
                # 趋势增强：可放松（TP 上调，SL 不放宽，保持/开启移动止盈）
                if new_tp > old_tp:
                    adjusted_tp = new_tp
            ...
            # ── 利润保护期：盈利超过 5% 后，止损至少锁定到成本价 ──
            # sl_pct 语义：pnl <= -sl_pct 时触发止损
            #   sl_pct = 0.06 → 亏 6% 止损
            #   sl_pct = 0.0  → 保本止损（pnl <= 0）
            #   sl_pct = -0.03 → 锁定 3% 利润（pnl <= +3% 时止损，即从高点回撤到只剩 3%）
            if pnl_pct >= 0.05:
                # 保本止损：SL 设为 0（pnl <= 0 触发）
                if adjusted_sl > 0:
                    adjusted_sl = 0.0
                    adjustment_log.append(f"利润保护:SL锁成本价(pnl={pnl_pct*100:.1f}%)")
                # 盈利超过 8% 后，锁定 3% 利润（sl_pct = -0.03）
                if pnl_pct >= 0.08:
                    locked_sl = -0.03
            ...
            if adjustment_log:
                ...
                self._save_position_meta()
                # 同步交易所 SL/TP 硬单（取消旧单 → 挂新单）
                self._sync_sltp_orders(symbol, meta)
            else:
                # 无调整但记录最新波浪状态（首次还会写入 wave_conf_at_open，需持久化）
                meta["wave_conf_current"] = wave_conf
                self._save_position_meta()

    def _sync_sltp_orders(self, symbol: str, meta: dict):
        """同步交易所 SL/TP 硬单到 meta 中的最新 sl_pct/tp_pct

        流程：取消旧挂单 → 挂新 SL/TP 单 → 记录 order_id 到 meta

        价格公式（sl_pct 语义：pnl <= -sl_pct 时触发）：
            多头: sl_price = entry_px * (1 - sl_pct)
                  sl_pct=0.06 → entry*0.94 (亏6%止损)
                  sl_pct=0    → entry       (保本)
                  sl_pct=-0.03 → entry*1.03 (锁3%利润)
            空头: sl_price = entry_px * (1 + sl_pct)
        """
        ...
        # 2. 挂止损单（始终挂，包括保本 sl_pct=0 和锁利润 sl_pct<0）
        if side == "long":
    ...
    def _handle_entry(self, symbol, action, direction, confidence, full_signal):
        ...
        try:
            ...
            position_pct = full_signal.get("position", {}).get("position_pct", 0.05)
