...
class HalvingTopExitStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        super().__init__(name="halving_top_exit_v4")
    ...
    def _get_halving_phase(self, current_date: pd.Timestamp) -> str:
        ...
        Returns:
            'normal' - 正常阶段
            'warn' - 预警区（减半后12-15个月）
            'danger' - 高危区（减半后15-18个月）
            'peak' - 见顶区（减半后18-24个月）
            'normal' - 减半后24个月以上恢复正常
        """
            ...
            if self.is_btc:
                ...
                if price_above_ma200 and slope_pos:
                    ...
                    if self.weekly_ma200_dip_buy and not np.isnan(weekly_ma200[i]) and weekly_ma200[i] > 0:
                        ...
                        # === 扩展抄底触发范围 ===
                        # 原逻辑：仅在价格低于周线MA200时触发抄底
                        # 问题：价格在周线MA200附近（上方0~5%）反复穿越时，
                        #        抄底频繁启停，导致在关键支撑位附近仍保持高仓位做空
                        # 修复：在周线MA200上方5%范围内也标记为"支撑区域"，
                        #        触发轻度抄底仓位（防止在支撑区域高仓位做空）
                        ...
                        if weekly_below_pct > 0:
                            ...
                            # 价格回升到周线MA200上方但仍在5%支撑区域内：
                            # 保持轻度抄底仓位（而非直接切换到高仓位做空）
                            # 仓位按距周线MA200的距离线性衰减
                    ...
                    if dip_buy_pos > 0:
                        ...
                        # === 支撑区域内保持 dip_buy_active ===
                        # 价格仍在周线MA200附近（5%以内）时，不清除 dip_buy_active
                        # 这样下次价格回落到周线MA200下方或5%以内时，
                        # 扩展保护逻辑仍然生效
...
if __name__ == "__main__":
    ...
    strategy = HalvingTopExitStrategy(is_btc=True)
