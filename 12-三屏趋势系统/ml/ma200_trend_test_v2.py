...
def fetch_okx_candles(inst_id, bar="1D", days=730):
    """直接用requests获取OKX历史K线（带重试）"""
    all_candles = []
    seen_ts = set()
    after = None
    page = 0
    max_pages = (days // 300) + 10

    print(f"  获取 {inst_id} {bar} 数据，目标 {days} 天...")
    ...
    while page < max_pages:
        ...
        for retry in range(3):
            try:
                ...
                if r.get("code") != "0":
                    print(f"  [WARN] 第{page+1}页错误: {r.get('msg')}")
                    time.sleep(1)
                    continue
    ...
    print(f"  获取到 {len(all_candles)} 条")
    return all_candles


def fetch_real_data(inst_id, days=730):
    candles = fetch_okx_candles(inst_id, "1D", days)
    if not candles:
        return pd.DataFrame()
    df = pd.DataFrame(candles)
    df["timestamp"] = pd.to_datetime(df["ts"], unit="ms")
    df = df.set_index("timestamp")
    return df[["o", "h", "l", "c", "vol"]].rename(columns={"o": "open", "h": "high", "l": "low", "c": "close", "vol": "volume"})
...
def main():
    ...
    for inst_id, name in symbols:
        ...
        prices = fetch_real_data(inst_id, days=1500)
        if prices.empty:
            print(f"  数据获取失败")
            continue
        ...
        strategies = [
            ...
            ("MA100牛熊", lambda: MA200TrendFollowingStrategy(
                ma_period=100, slope_period=5, max_position=1.0, warmup_periods=110
            )),
    ...
    for sname in ["MA200牛熊", "MA200(3日斜率)", "MA200(10日斜率)", "MA150牛熊", "MA100牛熊", "综合最优LR", "双均线20/200", "买入持有"]:
