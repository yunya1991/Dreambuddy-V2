...
三屏趋势系统 = 「BTC风向标闸门 + 趋势方向判定 + 置信度评估 + 价值风险评估 + 仓位/加仓计算」
...
| 仓位/加仓计算 | 本模块 | 6档置信度映射 + 逆势/顺势加仓 + 逐仓5x杠杆 |
...
优先级0: BTC风向标闸门（宏观方向过滤）
    → 优先级1: 趋势一致性检测（Screen 1，周线+日线）
        → 优先级2: 置信度评估（Screen 2，贝叶斯+经典+Freqtrade）
            → 优先级3: 五大算法 final_signal（规则决策）
                → 优先级4: 集成推理层（LightGBM 集成 + LLM 辩证推理）
                    → 优先级5: 价值风险评估（仓位调整）
```
...
│   ├── feature_engineer.py             #    价格特征工程（5维）
│   ├── algo_ensemble.py                # ⭐ LightGBM集成推理（五大算法46维特征）
│   ├── llm_reasoning.py                # ⭐ LLM辩证推理（DeepSeek，按需触发）
│   ├── label_samples.py                # ⭐ 样本标注 + 一键训练工具
│   ├── tuner.py                        #    超参调优
...
### 6.4 价值风险与加仓 API (core/risk_reward.py)
