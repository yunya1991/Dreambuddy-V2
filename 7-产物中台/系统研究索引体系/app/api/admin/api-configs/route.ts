...
export async function GET(request: Request) {
  try {
    ...
    const { items, total } = await getAdminApiConfigList(q.page, q.pageSize);
    return NextResponse.json(
      apiSuccess(items, { meta: calculateMeta(total, q.page, q.pageSize) }),
    );
  } catch (error) {
    return NextResponse.json(apiError("获取API配置失败", error instanceof Error ? error.message : String(error)));
  }
}
