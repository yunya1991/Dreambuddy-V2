...
> **来源**: 对话归档（finance：订单簿, 流动性, 资金费率）
> **标签**: #订单簿 #流动性 #资金费率
> **状态**: active

## 调研结论

P1-20260805-02，某次紧急修复中临时关闭 Schema 检查开关但忘记恢复，随后 14 天写入的 `funding_rate` 列单位由小数（0.0001）被误写为万分比（0.01），导致资金费率因子放大 100 倍，五域「天」域波动率分位出现系统性偏移；纠正方案：Schema 验证开关仅在开发态生效，生产态移除旁路代码。**反模式 D**——「清洗链 VIFDropper 小样本保护
